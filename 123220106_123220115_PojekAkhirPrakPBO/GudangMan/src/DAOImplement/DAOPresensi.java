...
public class DAOPresensi implements DAOPresensiInterface {
    ...
    final String search = "SELECT * FROM presensi INNER JOIN tukang ON presensi.idTukang = tukang.id WHERE nama LIKE ?";
    ...
    @Override
    public List<DataPresensi> search(String nama) {
        List<DataPresensi> dataPresensi = null;
        try {
            dataPresensi = new ArrayList<DataPresensi>();
            ...
            while (result.next()) {
                ...
                presensi.setSb(result.getString("presensi.sb"));
                presensi.setMg(result.getString("presensi.mg"));
                presensi.setSubTotal(result.getInt("presensi.subtotal"));
                dataPresensi.add(presensi);
            }
            statement.close();
        } catch (SQLException ex) {
